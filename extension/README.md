# PHẢN ỨNG. — Extension Chrome (v1.3.0)
...
## Bảng đen qua menu chuột phải (mới)

Chuột phải vào **bất kỳ vị trí nào** trên bất kỳ trang web nào → chọn **"📝 Mở bảng đen tại
đây"** để bật một cửa sổ Bảng đen nổi ngay trên trang đó, ngay cạnh vị trí bạn vừa bấm chuột phải:

- **Kéo di chuyển**: giữ và kéo thanh tiêu đề của cửa sổ.
- **Tuỳ chỉnh kích thước**: kéo góc dưới-phải của cửa sổ (dùng tính năng resize gốc của trình
  duyệt, không giới hạn tỉ lệ khung hình).
- **Đóng**: bấm nút ✕ ở góc trên-phải cửa sổ, hoặc bấm "← Thoát bảng đen" bên trong bảng.
- Bên trong là đúng tính năng Bảng đen của trang chính (bút, tẩy, đổi màu phấn, hoàn tác/làm
  lại, gợi ý nhận diện phương trình) — vì nội dung được nhúng trực tiếp từ
  `https://sfes-lab.netlify.app/?embed=1&room=blackboard`.
- Có thể mở nhiều lần trên cùng một trang; cửa sổ dùng lại, chỉ di chuyển đến vị trí chuột phải
  mới nhất mỗi lần mở.

Menu chuột phải này được đăng ký trong `background.js` (quyền `contextMenus`), còn việc vẽ và
điều khiển cửa sổ nổi (kéo/resize/đóng) nằm trong `content.js`.
...
- Cửa sổ Bảng đen nổi (menu chuột phải) cũng nhúng cùng domain Netlify này, nên **trang
  `sfes-lab.html`/`index.html` cần được deploy lên Netlify với bản mới nhất** (đã có hỗ trợ
  `?embed=1&room=blackboard`) thì tính năng chuột phải mới hoạt động.

**Nếu bạn đổi domain Netlify**, chỉ cần sửa lại hằng số `APP_URL` ở đầu 3 file:
...
4. Xong — icon ⚗ sẽ xuất hiện trên thanh công cụ, tính năng bôi đen và menu chuột phải "Mở bảng
   đen tại đây" sẽ hoạt động trên mọi trang (cần có kết nối internet vì nội dung được tải từ
   Netlify). Nếu extension đã cài từ trước, hãy tải lại trang web đang mở (F5) sau khi bấm ⟳ để
   content script mới (có tính năng bảng đen) được nạp lại.

## Cấu trúc thư mục

```
extension/
├── manifest.json      Khai báo Manifest V3 (đã thêm quyền "contextMenus")
├── background.js      Bấm icon → mở tab mới; đăng ký & xử lý menu chuột phải "Mở bảng đen tại đây"
├── content.js          Chạy nền trên mọi trang: phát hiện vùng bôi đen (icon/panel nổi) và
│                        vẽ/điều khiển cửa sổ Bảng đen nổi (kéo, resize, đóng) khi được gọi từ
│                        menu chuột phải. Cả hai đều dùng <iframe> trỏ về
│                        https://sfes-lab.netlify.app/?embed=1&...
└── icons/              Icon toolbar
```
...
- **Một số trang web có thể chặn iframe từ domain khác** (qua header `Content-Security-Policy:
  frame-ancestors` hoặc `X-Frame-Options`) — khi đó panel nổi (bôi đen) hoặc cửa sổ Bảng đen nổi
  sẽ không hiện được nội dung. Đây là giới hạn từ phía trang web đó, không sửa được từ extension.
  Trong trường hợp này, bấm icon extension trên toolbar để mở toàn bộ ứng dụng trong tab riêng
  vẫn hoạt động bình thường.
- **Cửa sổ Bảng đen nổi** không mở được trên các trang nội bộ của Chrome (`chrome://...`), trang
  Chrome Web Store, hoặc trình xem PDF tích hợp — đây là giới hạn bảo mật của Chrome đối với mọi
  content script, không riêng extension này.
